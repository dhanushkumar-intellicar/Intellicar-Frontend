<script>
  ...
  async function handleSubmit() {
    responseData = {
      success: true,
      data: {
        channels: selectedChannels.map((c) => `CH${c + 1}`),
    ...
    // Show snackbar
    snackbar.set({ show: true, message: "Request sent successfully!", type: "success" });
  ...
  <div class="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      <div class="flex flex-wrap gap-2">
        {#each [0,1,2,3] as ch}
          <label class="flex items-center gap-1 px-2 py-1 bg-white rounded-lg shadow-sm hover:shadow-md cursor-pointer transition">
            <input type="checkbox" bind:group={selectedChannels} value={ch} class="accent-blue-600 scale-105"/>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
      <div>
        ...
        <input type="datetime-local" bind:value={startTime} class="w-full px-2 py-1 rounded-lg border focus:ring-1 focus:ring-blue-400 outline-none text-sm"/>
      </div>
      <div>
        <label class="text-sm font-medium">End Time</label>
        <input type="datetime-local" bind:value={endTime} class="w-full px-2 py-1 rounded-lg border focus:ring-1 focus:ring-blue-400 outline-none text-sm"/>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      <div>
        <label class="text-sm font-medium">Stream Type</label>
